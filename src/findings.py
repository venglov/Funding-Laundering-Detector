...
class FundingLaunderingFindings:
    ...
    def funding(from_, to, usd, token, is_new, type_, tx_hash):
        return Finding({
            ...
            'type': FindingType.Suspicious if get_severity(usd) != FindingSeverity.Low or is_new else FindingType.Info,
            'metadata': {
                'funded_address': to,
    ...
    def laundering(from_, to, usd, token, is_new, type_, tx_hash):
        return Finding({
            ...
            'type': FindingType.Suspicious if get_severity_laundering(usd) != FindingSeverity.Low else FindingType.Info,
